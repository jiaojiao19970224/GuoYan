...
class ConstantVelocityAgent(BasicAgent):
    """
    ConstantVelocityAgent implements an agent that navigates the scene at a fixed velocity.
    This simplified version does not include collision avoidance or traffic light response.
    """

    def __init__(self, vehicle, target_speed=20, opt_dict={}, map_inst=None, grp_inst=None):
        """
        Initializes the agent with constant velocity behavior.

            :param vehicle: actor to apply to agent logic onto
            :param target_speed: speed (in Km/h) at which the vehicle will move
            :param opt_dict: dictionary for additional options (optional).
            :param map_inst: carla.Map instance (optional).
            :param grp_inst: GlobalRoutePlanner instance (optional).
        """
        super().__init__(vehicle, target_speed, opt_dict=opt_dict, map_inst=map_inst, grp_inst=grp_inst)

        self._target_speed = target_speed / 3.6  # Convert target speed to m/s
        self.is_constant_velocity_active = True
        self._set_constant_velocity(self._target_speed)
    ...
    def run_step(self):
        """Execute one step of navigation without any safety constraints."""
        if self.is_constant_velocity_active:
            # Apply constant velocity
            self._set_constant_velocity(self._target_speed)

        # The longitudinal PID is still applied to maintain wheel dynamics
        control = self._local_planner.run_step()
        return control

    def destroy_sensor(self):
        """Placeholder for sensor destruction, if needed."""
        pass

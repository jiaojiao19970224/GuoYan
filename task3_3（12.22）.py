...
def decide_lane_change_intent(trend_speed_benefit, trend_space_benefit, f_necessity,
    ...
    best_benefit = -float('inf')
        ...
        # 根据车道位置调整允许的意图范围
    allowed_intents = []
    if current_lane == 1:  # 最左侧车道
        allowed_intents = [1, 2]  # 保持或向右换道
    elif current_lane == 2:  # 最右侧车道
        allowed_intents = [0, 1]  # 保持或向左换道
    else:  # 中间车道
        allowed_intents = [0, 1, 2]  # 向左、保持或向右换道
...
last_random_change_time = 10  # 初始化变量

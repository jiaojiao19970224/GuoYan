...
class Task4_3:
    ...
    def compare_gap_and_right_of_way(self, leading_vehicle, following_vehicle):
        ...
        following_vehicle_pos = (
        self._vehicle_info[following_vehicle]['lon'], self._vehicle_info[following_vehicle]['lat'])
    ...
    # 新增纵向速度判断函数
    def evaluate_front_vehicle(self, host_vehicle_id):
        # 找到当前车道前车ID
        front_vehicle_id = self.find_front_vehicle_on_current_lane(host_vehicle_id)
        host_vehicle = self._vehicle_info.get(host_vehicle_id)
        ...
        host_speed = host_vehicle['speed']

        # 如果没有前车，则保持不变
        if front_vehicle_id is None:
            return None, 5

        # 从 _vehicle_info 字典中获取前车信息
        front_vehicle = self._vehicle_info.get(front_vehicle_id)
        ...
        relative_speed = host_speed - front_speed
        ...
        # 此次根据规则
        if ttc < self._min_ttc:
            # 碰撞时间过短需减速
            command = 6
        else:
            # 距离足够，则根据与前车速度关系来定策略
            if front_speed > host_speed:
                command = 4  # 加速
            elif front_speed < host_speed:
                # 前车较慢但距离足够，这里保持不变
                command = 5
            else:
                # 速度相同也保持不变
    ...
    def main(self):
        ...
        front_speed, vertical_behavior = self.evaluate_front_vehicle(host_vehicle_id)
        ...
        if not current_lane_ttc_satisfied or not target_lane_ttc_satisfied or not target_lane_back_ttc_satisfied:
            ...
            vehicle_behavior_priority = {
                host_vehicle_id: {
                    "behavior": 1,
                    "priority": 1,
                    "vertical_behavior": vertical_behavior,
                    "front_vehicle_speed": front_speed
                }
            }
            return vehicle_behavior_priority
            ...
            if self.compare_gap_and_right_of_way(target_front_vehicle, target_rear_vehicle) == 1:
                # print("实际空隙更大")
                conflict_vehicle = self.check_conflict_vehicles(relevant_lanes, target_lane, host_lane_index,
                                                                host_vehicle_id, host_intention)
                if conflict_vehicle == 1:
                    ...
                    if self._coopSet:
                        ...
                        for vehicle_id in self._coopSet:
                            if vehicle_id == host_vehicle_id:
                                vehicle_behavior_priority[vehicle_id] = {
                                    "behavior": host_intention,
                                    "priority": vehicle_behavior_priority[vehicle_id]["priority"],
                                    "vertical_behavior": vertical_behavior,
                                    "front_vehicle_speed": front_speed
                                }
                            else:
                                vehicle_behavior_priority[vehicle_id] = {
                                    "behavior": 1,
                                    "priority": vehicle_behavior_priority[vehicle_id]["priority"],
                                    "vertical_behavior": 5,  # 对其它车辆，纵向行为默认保持不变

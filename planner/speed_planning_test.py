...
def speed_DP(obs_st_s_in_set, obs_st_s_out_set, obs_st_t_in_set, obs_st_t_out_set, plan_start_s_dot,
    ...
    #     pre_col = min_col - 1
    #
    #     # 确保 pre_row 和 pre_col 在合理范围内
    #     if pre_row < 0 or pre_row >= len(s_list):
    #         raise IndexError(f"pre_row index {pre_row} out of bounds for s_list of length {len(s_list)}")
    #     if pre_col < 0 or pre_col >= len(t_list):
    #         raise IndexError(f"pre_col index {pre_col} out of bounds for t_list of length {len(t_list)}")
    #
    #     dp_speed_s[pre_col], dp_speed_t[pre_col] = CalcSTCoordinate(pre_row, pre_col, s_list, t_list)
    #     min_row = pre_row
    #     min_col = pre_col
    ...
    while min_col != 0:
        ...
        pre_col = int(min_col - 1)

        # 确保 pre_row 和 pre_col 在合理范围内
        if pre_row < 0 or pre_row >= len(s_list):
            logging.error(f"pre_row index {pre_row} out of bounds for s_list of length {len(s_list)}")
            raise IndexError(f"pre_row index {pre_row} out of bounds for s_list of length {len(s_list)}")
        if pre_col < 0 or pre_col >= len(t_list):
            logging.error(f"pre_col index {pre_col} out of bounds for t_list of length {len(t_list)}")
            raise IndexError(f"pre_col index {pre_col} out of bounds for t_list of length {len(t_list)}")

        dp_speed_s[pre_col], dp_speed_t[pre_col] = CalcSTCoordinate(pre_row, pre_col, s_list, t_list)
        # dp_speed_s[pre_row], dp_speed_t[pre_col] = CalcSTCoordinate(pre_row, pre_col, s_list, t_list)
        logging.debug(
            f"Backtracked to pre_row: {pre_row}, pre_col: {pre_col}, dp_speed_s: {dp_speed_s[pre_col]}, dp_speed_t: {dp_speed_t[pre_col]}")
        min_row = pre_row
        min_col = pre_col
...
#     # 确保row在合理范围内
#     if row < 0 or row >= m:
#         raise IndexError(f"row index {row} out of bounds for s_list of length {m}")
#
#     # 0,0 对应的是 s的最大值 t的最小值
#     s_value = s_list[m - row - 1]  # 注意s_list的索引
#     t_value = t_list[col]  # 注意t_list的索引
#
#     return s_value, t_value
...
def CalcSTCoordinate(row, col, s_list, t_list):
    ...
    # 确保 row 在合理范围内
    if row < 0 or row >= m:
        logging.error(f"row index {row} out of bounds for s_list of length {m}")
        raise IndexError(f"row index {row} out of bounds for s_list of length {m}")

    # 确保 col 在合理范围内
    if col < 0 or col >= len(t_list):
        logging.error(f"col index {col} out of bounds for t_list of length {len(t_list)}")
        raise IndexError(f"col index {col} out of bounds for t_list of length {len(t_list)}")

    # 0,0 对应的是 s 的最大值 t 的最小值
    s_value = s_list[m - row - 1]  # 注意 s_list 的索引
    t_value = t_list[col]  # 注意 t_list 的索引

    logging.debug(f"Calculated s_value: {s_value}, t_value: {t_value} for row: {row}, col: {col}")
...
def speed_QP(plan_start_s_dot, plan_start_s_dot2, dp_speed_s, dp_speed_t, s_lb, s_ub, s_dot_lb, s_dot_ub,
    ...
    # dp_speed_end = len(dp_speed_s) - 1  # 确保索引在范围内
    ...
    # 打印 Aeq 和 beq 的尺寸以进行调试
    print(f"Aeq matrix shape: {Aeq.size}")
    print(f"beq matrix shape: {beq.size}")
    ...
    print(f"A matrix shape: {A.size}")
    print(f"b matrix shape: {b.size}")

    # assert A.size[1] == 33, f"Expected A matrix to have 33 columns but got {A.size[1]}"
...
#     ub = cvxopt.matrix(ub)
#
#     sol = cvxopt.solvers.qp(H, f, A, b, Aeq, beq)
#     X = np.array(sol['x'])
#
#     for i in range(1, qp_size + 1):
#         qp_s_init[i - 1] = X[3 * i - 3]
